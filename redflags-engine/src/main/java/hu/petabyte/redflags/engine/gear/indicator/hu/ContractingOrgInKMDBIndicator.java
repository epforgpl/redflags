 ...
 */
@Component
@ConfigurationProperties(prefix = "contractingOrgInKMDBIndicator")
public class ContractingOrgInKMDBIndicator extends AbstractTD3CIndicator {
	...
	protected IndicatorResult flagImpl(Notice notice) {
		if (null != notice.getContr().getContractingOrg()) {
			String name = notice.getContr().getContractingOrg().getName();
			String found = kmdb.findOrgName(name);
			if (null != found) {
				return returnFlag("info", "tedOrg=" + name, "kmdbOrg=" + found);
			}
		}
		return null;
	}
